...
const PUBLIC_MCP_CODEX_NAME = "netcatty-public";
...
function classifyCodexPublicMcpStatus({ entries, launcherPath, codexPath }) {
  const commandArgs = ["mcp", "add", PUBLIC_MCP_CODEX_NAME, "--", launcherPath];
  const base = {
    ok: true,
  ...
  const entry = Array.isArray(entries)
    ? entries.find((item) => item?.name === PUBLIC_MCP_CODEX_NAME)
    : null;
...
function createPublicMcpCodexSetup(options = {}) {
  const deps = {
  ...
  function getManualCommand() {
    return formatCodexCommandText(["mcp", "add", PUBLIC_MCP_CODEX_NAME, "--", deps.launcherPath]);
  }
  ...
  async function getStatus() {
    ...
    try {
      ...
      return classifyCodexPublicMcpStatus({
