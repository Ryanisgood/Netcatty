...
const PUBLIC_MCP_CLAUDE_NAME = "netcatty-public";
...
function isMissingClaudeServer(result) {
  return /No MCP server found with name:\s*["']?netcatty-public["']?/i.test(getCombinedOutput(result));
}
...
function extractExistingCommand(result) {
  ...
  const matchingLine = output
    .split(/\r?\n/u)
    .map((line) => line.trim())
    .find((line) => line.includes(PUBLIC_MCP_CLAUDE_NAME));
...
function classifyClaudePublicMcpStatus({ getResult, launcherPath, claudePath }) {
  const commandArgs = ["mcp", "add", PUBLIC_MCP_CLAUDE_NAME, "--", launcherPath];
  const base = {
    ok: true,
...
function createPublicMcpClaudeSetup(options = {}) {
  const deps = {
  ...
  function getManualCommand() {
    return formatClaudeCommandText(["mcp", "add", PUBLIC_MCP_CLAUDE_NAME, "--", deps.launcherPath]);
  }
  ...
  async function getStatus() {
    ...
    try {
      const result = await runClaude(claudePath, shellEnv, ["mcp", "get", PUBLIC_MCP_CLAUDE_NAME]);
      return classifyClaudePublicMcpStatus({
        getResult: result,
        launcherPath: deps.launcherPath,
        claudePath,
      });
